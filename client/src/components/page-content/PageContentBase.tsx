...
const PageContentBase: React.FC<PageContentBaseProps> = ({ children, title, subtitle, useBack, backTo, actions, aside }) => {
  const history = useHistory();
  const backAction = useBack ? () => { backTo ? history.push(backTo) : history.goBack() } : undefined;
        ...
        aside ? (
          <>
            <Col xl={18} xxl={18} lg={24} md={24} sm={24} xs={24}>
              <CardWithBoxShadow
                title={<PageHeaderWithoutPadding title={title} subTitle={subtitle} onBack={backAction} extra={actions} />}

                actions={[
                  <Text key={nanoid()} type='secondary'>{t('RESOURCES.COMMON.DATETIME_HINT', { timeZone: dateTimeUtils.displayedCurrentTimezoneInfo() })}</Text>,
                  <b key={nanoid()}>aaaa</b>
                ]}
              >
                {
