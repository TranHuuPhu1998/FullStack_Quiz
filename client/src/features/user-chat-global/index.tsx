...
const UserCharGlobal:React.FC = () => {
  ...
  const chats = useSelector((state:RootState) => state.chatGlobalReducers)
  ...
  const handleLoadMore = useCallback(() => {
    if (stop) return;
    setPageInfo(prev => ({ ...prev, page: prev.current + 1 }))
  }, [stop])
  ...
  useEffect(() => {
    socketRef.current = io('https://quiz-app-hihi.herokuapp.com/');
